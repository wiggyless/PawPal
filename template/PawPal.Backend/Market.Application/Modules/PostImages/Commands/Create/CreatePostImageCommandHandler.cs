...
{
    public class CreatePostImageCommandHandler(IAppCurrentUser user, IAppDbContext context, IFileStorageService fileStorage) :
        IRequestHandler<CreatePostImageCommand,int>
    {
        public async Task<int> Handle(CreatePostImageCommand command,CancellationToken cancellationToken)
        {
            ...
            var savedPaths = await fileStorage.SaveFilesAsync(
                command.PostImages, $"posts/Post_{command.PostId}", cancellationToken);
