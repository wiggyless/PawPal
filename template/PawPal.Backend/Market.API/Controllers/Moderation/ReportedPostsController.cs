...
namespace PawPal.API.Controllers.Moderation
{
    ...
    public class ReportedPostsController(ISender sender) : ControllerBase
    {
        ...
        public async Task<GetReportedPostByIdQueryDto> GetById(int id, CancellationToken cancellationToken)
        {
            var report = await sender.Send(new GetReportedPostByIdQuery { Id = id }, cancellationToken);
            return report;
        }
        [HttpPost]
